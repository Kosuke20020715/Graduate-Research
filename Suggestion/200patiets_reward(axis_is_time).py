...
def offline_optimize_model(memory, GAMMA,e,policy_net,target_net, optimizer):
    transitions = memory.sample(BATCH_SIZE)
    batch = list(zip(*transitions))
    states_batch = torch.tensor(batch[0], dtype=torch.float32).to(device)
    next_states_batch = torch.tensor(batch[3], dtype=torch.float32).to(device)
    # print(batch[1])
    actions_batch = torch.as_tensor(batch[1], dtype=torch.int64).to(device)
    rewards_batch = torch.tensor(batch[2], dtype=torch.float32).to(device)
    # print(actions_batch)
    new_action_batch = multidim_to_1d(actions_batch).unsqueeze(1)
    Q_values = policy_net(states_batch).gather(1, new_action_batch).squeeze(1)
    # print(policy_net(states_batch),new_action_batch)
    with torch.no_grad():
        next_Q_values = target_net(next_states_batch).detach().max(1)[0]
...
def online_optimize_model(memory, GAMMA,e,policy_net,target_net, optimizer):
    transitions = memory.sample(BATCH_SIZE)
    batch = list(zip(*transitions))
    states_batch = torch.tensor(batch[0], dtype=torch.float32).to(device)
    next_states_batch = torch.tensor(batch[3], dtype=torch.float32).to(device)
    ...
    actions_batch = torch.as_tensor(batch[1], dtype=torch.int64).to(device)
    rewards_batch = torch.tensor(batch[2], dtype=torch.float32).to(device)
    new_action_batch = actions_batch.to(dtype=torch.int64).unsqueeze(1)
    # print(new_action_batch)
    Q_values = policy_net(states_batch).gather(1, new_action_batch).squeeze(1)
    # print(policy_net(states_batch),new_action_batch)
    with torch.no_grad():
        next_Q_values = target_net(next_states_batch).detach().max(1)[0]
...
def process_per_groups():
    ...
    for group, df_group in patient_data.groupby('PtID'):
        ...
        for index, row in df_group.iterrows():
            ...
            if csv_action[5] > 1:
                csv_action[5] = 1
            offline_memory.push(states, csv_action[:5], csv_reward, next_states)
            online_memory.push(states,action_list.index(csv_action[5]),csv_reward, next_states)
            ...
            if count > BATCH_SIZE and count % 20 ==0 :
                offline_optimize_model(offline_memory, GAMMA, count, offline_policy_net,offline_target_net, offline_optimizer)
                online_optimize_model(online_memory, GAMMA, count, online_policy_net,online_target_net, online_optimizer)
...
def online_train(X_0, V_0, Subtype, episode): #onlineのトレーニング
    ...
    for e in range(0, episode, 7):
        state = [X, V, Subtype, e, e+7] #state+subtype+date/date+7
        ...
        for i in range(7):
            ...
            total_reward += reward
            offline_memory.push(state, decoded_action, reward, next_state) 
            online_memory.push(state, action_list.index(action), reward, next_state) 
            # print(action_list.index(action))
            state = next_state
        prev_action = action 
        GAMMA = 0.5
        if e > BATCH_SIZE and e % 10 == 0:
            offline_optimize_model(offline_memory, GAMMA, e, offline_policy_net, offline_target_net, offline_optimizer) 
            online_optimize_model(online_memory, GAMMA, e, online_policy_net, online_target_net, online_optimizer)
        # 指数減少
...
for state in state_list:
    epsilon = epsilon_start
    X_vals, V_vals, actions, reward_record, actions_all,total_action, offline_actions, total_reward_per_day = online_train(state[0],state[1],state[2],140)#60日に1度観測可能
...
ax.set_xticks(np.arange(0, total_days + 1, 7))
ax.set_xticklabels(np.arange(0, total_days + 1, 7))
ax.set_xlabel("Days")
